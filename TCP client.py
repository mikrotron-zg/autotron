...
def Command(command):
	try:									# hvataj gresku!
		time.sleep(0.05)					# zastita od DoS efekta jer je server na robotu sporiji od klijenta, proizvoljno/empirijski - pokusom utvrdjena konstanta upravljanja ~5ms
		print "connecting..."
...
while run:
	...
	if keys[pygame.K_UP]!=0:
		cmd="f"+chr(200)						# ako je pritisnuta tipka gore, naredba je "f"(forward) i char 100 ("d") koji se tumaci kao brzina
	if keys[pygame.K_DOWN]!=0:
		cmd="b"+chr(200)						# analogno "b"(back)
	if keys[pygame.K_LEFT]!=0:
		cmd="l"+chr(200)						# "l"(left)
	if keys[pygame.K_RIGHT]!=0:
		cmd="r"+chr(200)						# "r"(right)
